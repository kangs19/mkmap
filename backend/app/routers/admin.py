...
async def collect_regional_prices_now(
    ...
    """KAMIS 지역별 도매/소매 가격 수집 — 최근 N일 (기본 7일).
    ...
    api_key = settings_.kamis_api_key
    if not api_key:
        return {"error": "KAMIS_API_KEY not configured"}
    ...
    async def _fetch(mc, target_date, sess):
        date_str = target_date.strftime("%Y-%m-%d")
        params = {"action":"dailySalesList","p_cert_key":api_key,"p_cert_id":"5300",
        ...
        for row in data.get("price", []):
            ...
            if pno not in PNO_TO_ITEM: continue
            cls = row.get("product_cls_code","")
            pstr = str(row.get("dpr1","")).replace(",","").strip()
            if not pstr or pstr=="-": continue
            try: price=float(pstr)
            except ValueError: continue
            if price<=0: continue
            grouped.setdefault(pno,{})
            if cls=="02": grouped[pno]["w"]=price
            elif cls=="01": grouped[pno]["r"]=price
        result=[]
        for pno, pd_ in grouped.items():
            w=pd_.get("w"); rv=pd_.get("r")
            if w is None and rv is None: continue
            if w is None: w=round(rv/1.35,0)
            if rv is None: rv=round(w*1.35,0)
            result.append({"item_code":PNO_TO_ITEM[pno],"date":target_date,
    ...
    async with httpx.AsyncClient(timeout=20, verify=False) as sess:
        cur = start_d
        while cur <= end_d:
            tasks = [_fetch(mc, cur, sess) for mc in REGIONAL_MARKETS]
            ...
            if rows_:
                ...
                stmt_ = stmt_.on_conflict_do_update(
                    ...
                    set_={"market_name":stmt_.excluded.market_name,
